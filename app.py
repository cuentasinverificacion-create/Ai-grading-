...
if uploaded_file is not None:
    ...
    if st.button("🚀 Iniciar Gradeo Inteligente"):
     ...
     # BUSCA ESTA PARTE EN TU CÓDIGO Y CÁMBIALA POR ESTA NUEVA:
...
"messages": [
  {
    ...
    "content": [
      {
        "type": "text",
        "text": """Analiza la imagen adjunta. Sigue estas reglas estrictas:
        1. Comprueba si la imagen es una carta Pokémon real (frente o reverso). Si NO es una carta Pokémon, pon en el campo 'defects' el texto 'ERROR: No es una carta' y pon las notas de corners, edges y surface en 0.0.
        2. Si SÍ es una carta, analízala como un juez experto. Evalúa esquinas (corners), bordes (edges) y superficie (surface). Puntúa de 1.0 a 10.0.
        Devuelve SOLO un formato JSON así: {"corners": float, "edges": float, "surface": float, "defects": [string]}"""
      },
      {
        "type": "image_url",
        "image_url": { "url": f"data:image/jpeg;base64,{base64_image}" }
...
# BUSCA DONDE DICE "calcular_escalas" Y REEMPLAZA ESA PARTE FINAL POR ESTA:

if "ERROR: No es una carta" in vlm_res["defects"]:
    st.error("❌ La IA no detecta una carta Pokémon en la foto. Por favor, repite la foto apuntando bien a la carta sobre fondo oscuro.")
else:
    # Calcular veredictos reales si todo está bien
    psa_f, bgs_f, cgc_f = calcular_escalas(centrado_nota, centrado_str, vlm_res)
    ...
    # ... (el resto del código de subgrados se queda igual)
